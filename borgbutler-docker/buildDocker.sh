...
(cd app; docker build -t micromata/borgbutler .)

echo "docker tag ..... version"
echo "docker push micromata/borgbutler:version"
echo "docker push micromata/borgbutler:latest"
echo
echo
echo "Run 'docker run -v $HOME/BorgButler:/BorgButler -p 127.0.0.1:9042:9042 --name borgbutler micromata/borgbutler'"
